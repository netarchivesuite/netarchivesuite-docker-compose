...
class PrototypeRecordService(AbstractRecordService):
    def resolveFile(self, filename, filedir):
        parent = parser.get('PrototypeRecordService', 'path') # Currently inserted for testing 15/12
        return parent + '/' + filename #'/data/' + filename
...
if __name__ == "__main__":
    parser = ConfigParser()
    parser.read('conf/warcrecordservice.conf')
