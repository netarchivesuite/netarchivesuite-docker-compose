#!/usr/bin/python3
import cgitb, cgi, os, subprocess, urllib.parse, sys
...
class AbstractFileResolver:
    ...
    def main(self, collection_dict):
        ...
        try:
            ...
            exact = False
            if (exactfilename is not None and exactfilename.lower() == 'true'):
                exact = True
            data = self.resolveFile(pattern_decoded, filedir, exact)
            print("Status: 200")
...
if __name__ == "__main__":
    parser = ConfigParser()
    parser.read('conf/fileresolver.conf')
